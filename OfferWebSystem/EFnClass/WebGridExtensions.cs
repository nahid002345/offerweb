...
public static class WebGridExtensions
{
    ...
    string paginationStyle = null,
        bool IsPartialAjaxEnabled=false)
    {
        return PagerList(webGrid, mode, firstText, previousText, nextText, lastText, numericLinksCount, paginationStyle,IsPartialAjaxEnabled:false, explicitlyCalled: true);
    }
        ...
        bool explicitlyCalled)
    {
        ...
        if (ModeEnabled(mode, WebGridPagerModes.FirstLast))
        {
            ...
            {
                InnerHtml = GridLink(webGrid, webGrid.GetPageUrl(0), firstText,IsPartialAjaxEnabled)
            };
        ...
        if (ModeEnabled(mode, WebGridPagerModes.NextPrevious))
        {
            ...
            var part = new TagBuilder("li")
            {
                InnerHtml = GridLink(webGrid, webGrid.GetPageUrl(page), previousText, IsPartialAjaxEnabled)
            };
        ...
        if (ModeEnabled(mode, WebGridPagerModes.Numeric) && (totalPages > 1))
        {
            ...
            for (int i = first; i <= last; i++)
            {
                ...
                var part = new TagBuilder("li")
                {
                    InnerHtml = GridLink(webGrid, webGrid.GetPageUrl(i), pageText, IsPartialAjaxEnabled)
                };
                ...
                {
                    part.MergeAttribute("class", "waves-effect");
                }
                li.Add(part);
        ...
        if (ModeEnabled(mode, WebGridPagerModes.NextPrevious))
        {
            ...
            var part = new TagBuilder("li")
            {
                InnerHtml = GridLink(webGrid, webGrid.GetPageUrl(page), nextText, IsPartialAjaxEnabled)
            };
        ...
        if (ModeEnabled(mode, WebGridPagerModes.FirstLast))
        {
            ...
            var part = new TagBuilder("li")
            {
                InnerHtml = GridLink(webGrid, webGrid.GetPageUrl(lastPage), lastText, IsPartialAjaxEnabled)
            };
